...
def main():
	...
	def aggregate_list_qtl(group, type_id):
		...
		qtl_scores = filtered_group['qtl_score'].tolist() if not filtered_group.empty else np.nan
		features = filtered_group['feature'].tolist() if not filtered_group.empty else np.nan
		return qtl_scores,features
	...
	def custom_agg(group):
		...
		eqtl_scores, eqtl_features = aggregate_list_qtl(group, 'eqtl')
		pqtl_scores, pqtl_features = aggregate_list_qtl(group, 'pqtl')
		sqtl_scores, sqtl_features = aggregate_list_qtl(group, 'sqtl')
